...
# Check that viewer data exists
if [[ ! -f reports/viewer/papers_data.json ]]; then
    echo "[ERROR] Missing reports/viewer/papers_data.json. Run digest first." >&2
    exit 1
fi

# Only commit viewer-related files
viewer_files=(reports/viewer/index.html reports/viewer/app.js reports/viewer/styles.css reports/viewer/papers_data.json reports/viewer/favicon.svg)

changed=()
for path in "${viewer_files[@]}"; do
...
git commit -m "chore(viewer): update site data for ${DATE_TAG}" -- "${changed[@]}"
